...
I noticed a lot of the issues I had were caused by the frontend being rendered on the server because of the WebSocket connection (and it being pretty unreliable).
I fixed this by adding `export const ssr = false` in the `+layout.svelte` file, which fixed a lot of the issues I had with the frontend.
I did not look deeper into why that was until when I began the rewrite, which is why I didn't know why the other frameworks didn't work for me.
